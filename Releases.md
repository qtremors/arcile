...
| :--- | :--- | :--- |
| [v0.4.1 Beta](#v041-beta) | 2026-03-17 | Performance & Storage Optimizations |
...
Welcome to Arcile v0.4.1 Beta! This update is laser-focused on significant performance optimizations, fixing critical storage calculation bugs, and solidifying our custom architecture to make Arcile the fastest offline file manager possible.

Here is everything new and improved in this release.

## 🚀 What's New & Improved

### ⚡ Zero-Latency Storage Dashboard
No more waiting for storage bars to load!
- **Persistent Analytics Caching:** We've built a robust, persistent caching layer (`Context.cacheDir/analytics/`) for storage analytics. This means your Home Screen and Storage Dashboard now render with zero latency the moment you open the app.
- **Smart Invalidation:** The cache is highly intelligent. It updates silently in the background and only invalidates the specific volume's cache when you create, rename, delete, copy, or move files within Arcile.

### 🧠 Modern Native OS Optimizations
We've significantly reduced the app's memory footprint and CPU overhead by deeply integrating with modern Android APIs, specifically designed for high-performance data querying.
- **StorageStatsManager Integration (API 26+):** Manual file-by-file size calculations are gone. Arcile now securely queries Android's `StorageStatsManager` to retrieve exact image, video, and audio size metrics instantly.
- **Optimized MediaStore Fallbacks:** For categories outside of images/videos/audio, we've optimized our MediaStore queries with SQL-level MIME-type filtering, skipping thousands of redundant file checks.
- **MimeTypeMap Integration:** File categorization has been upgraded from a hardcoded list of extensions to utilizing the OS's native `MimeTypeMap` and `MediaStore.MediaColumns.MIME_TYPE`. This guarantees significantly improved accuracy and immediate support for newly recognized file formats.
- **Native Thumbnail Extraction:** Album art and image thumbnails are now processed via `ContentResolver.loadThumbnail()` (API 29+) taking full advantage of Android's system-level caching and background decoding.

### 🗑️ The Refined "Custom Trash" System
We originally planned to migrate to Android 11's native `MediaStore.createTrashRequest()`. However, testing revealed this OS feature forces highly intrusive, repetitive permission dialogs and frequently causes "double-trashing" bugs for full-access file managers.
- **Unified Custom Trash:** We have completely abandoned the Native OS Trash in favor of universally utilizing our lightning-fast, silent custom `.arcile/.trash` system across all Android versions.
- **No More Permission Loops:** Trashing, restoring, and emptying the bin now happens instantly and seamlessly without the OS interrupting you.

## 🐛 Bug Fixes

This release stomps out several frustrating bugs:
- **Storage Calculation Inaccuracy:** Fixed a major bug where storage category sizes (like Documents or Archives) were severely under-reported because the app was only counting the first file it found or missing MIME-less categories entirely.
- **Lingering Trashed Files:** Fixed a bug where files sent to the Trash Bin would lose their extension but still bizarrely show up in "Recent Files" and Category views. Arcile now aggressively deletes these entries from the MediaStore index upon trashing.
- **Orphaned Metadata:** Fixed an edge-case copy fallback bug in `moveToTrash` that would accidentally delete the `.json` metadata file if a fast-rename failed, which permanently orphaned the file. Files can now always be correctly restored.
- **Corrupted Metadata Crashes:** Added proactive logging and cleanup logic to safely ignore and discard invalid or corrupted trash JSON metadata, preventing the Trash screen from crashing during parsing.
- **Typed Error Handling:** We eliminated string-based error matching in favor of strongly-typed exceptions (`DestinationRequiredException`) for a much more robust architecture under the hood.

## ⚠️ Important Note: Android 10 Support

As we continue to optimize Arcile for absolute peak performance utilizing direct `java.io.File` APIs, Android 10 (API 29) is officially unsupported. Because Android 10 forces "Scoped Storage" but lacks the `MANAGE_EXTERNAL_STORAGE` permission (which was introduced in Android 11), the app cannot physically browse or manage global files on this specific OS version. Devices running Android 9 and below, or Android 11 and above, remain fully supported!

Thank you for testing Arcile Beta! If you encounter any issues, please check out our [Developer Documentation](https://github.com/qtremors/arcile/blob/main/DEVELOPMENT.md) or drop a report on the Issue Tracker.
