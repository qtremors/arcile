...
private data class ResolvedTransferEntry(
    val parent: ResolvedVaultDirectory,
    val snapshot: dev.qtremors.arcile.core.vault.crypto.VaultDirectorySnapshot,
...
private fun VaultCancellationSignal.throwIfCancelled() {
    if (isCancelled()) throw VaultFailure.Cancelled()
}

private fun findByName(entries: List<VaultManifestEntry>, name: String): VaultManifestEntry? {
    val key = VaultName.of(name).comparisonKey
    return entries.firstOrNull { VaultName.of(it.name).comparisonKey == key }
}

private fun VaultManifestEntry.conflictWith(source: VaultManifestEntry) = VaultConflict(
...
private fun uniqueName(name: String, entries: List<VaultManifestEntry>): String {
...
private fun result(ref: VaultNodeRef, entry: VaultManifestEntry, outcome: VaultItemOutcome) = VaultItemResult(
