...
Arcile is designed to operate entirely offline. The application does not declare the `android.permission.INTERNET` permission, which provides a hard technical guarantee that your files and activity data cannot leave your device.
...
Arcile requires the `MANAGE_EXTERNAL_STORAGE` permission (Android 11+) to perform file management operations such as browsing, copying, moving, and deleting files as initiated by you. All file operations are executed locally on your device — **no data is transmitted externally** under any circumstances.
